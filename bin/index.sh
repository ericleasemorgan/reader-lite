...
$MAKEFILESYSTEM "$DIRECTORY" "$IDENTIFIER"

# cache file
printf "Step #03 of 16: Caching original file\n" >&2
$CACHEFILE "$FILE" "$DIRECTORY/$IDENTIFIER"

# transform file into plain text
printf "Step #04 of 16: Transforming file into plain text\n" >&2
TEXT=$( $FILE2TXT "$FILE" )
echo $TEXT > "$DIRECTORY/$IDENTIFIER/$IDENTIFIER-text.txt"
echo $TEXT > "$DIRECTORY/$TXT/$IDENTIFIER.txt"

# re-configure
FILE="$DIRECTORY/$IDENTIFIER/$IDENTIFIER-text.txt"
...
$TXT2BIB "$FILE" > "$DIRECTORY/$IDENTIFIER/$IDENTIFIER-bib.tsv" &

# unigrams
printf "Step #06 of 16: Extracting unigrams\n" >&2
$TXT2NGRAMS 1 "$FILE" > "$DIRECTORY/$IDENTIFIER/$IDENTIFIER-unigrams.tsv" &
...
$TXT2NGRAMS 2 "$FILE" > "$DIRECTORY/$IDENTIFIER/$IDENTIFIER-bigrams.tsv" &

# trigrams
printf "Step #08 of 16: Extracting trigrams\n" >&2
$TXT2NGRAMS 3 "$FILE" > "$DIRECTORY/$IDENTIFIER/$IDENTIFIER-trigrams.tsv" &
...
$TXT2ADR "$FILE" > "$DIRECTORY/$IDENTIFIER/$IDENTIFIER-email.tsv" &

# URLs
printf "Step #10 of 16: Extracting URLs\n" >&2
$TXT2URLS "$FILE" > "$DIRECTORY/$IDENTIFIER/$IDENTIFIER-urls.tsv" &
...
$TXT2ENT "$FILE" > "$DIRECTORY/$IDENTIFIER/$IDENTIFIER-entities.tsv" &

# parts-of-speech
printf "Step #12 of 16: Extracting parts-of-speech\n" >&2
$TXT2POS "$FILE" > "$DIRECTORY/$IDENTIFIER/$IDENTIFIER-pos.tsv" &
...
# keywords
$TXT2KEYWORDS "$FILE" > "$DIRECTORY/$IDENTIFIER/$IDENTIFIER-keywords.tsv"

# various grammers; subjects-verbs-objects
printf "Step #13 of 16: Extracting subjects-verbs-objects\n" >&2
$TXT2SVO "$FILE" > "$DIRECTORY/$IDENTIFIER/$IDENTIFIER-svo.tsv" &
